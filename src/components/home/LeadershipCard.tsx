...
const LeadershipCard = ({ imageSrc, name, role, description }: LeadershipCardProps) => {
  return (
    <article className="overflow-hidden  md:grid md:grid-cols-2  bg-white shadow-[0_20px_60px_rgba(0,0,0,0.12)]">
      ...
      <div className="w-full  relative md:hidden h-80 overflow-hidden bg-[#347990]">
        <div><img
          src={imageSrc}
          alt={name}
          className="w-full scale-[40%] absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-[40%] object-cover object-top"
        /></div>
      ...
      <div className="bg-brand px-6 py-8 text-white sm:px-8">
        ...
        <div className="mt-6 space-y-1">
          ...
          <p className="text-base font-normal leading-6 text-white/80">{role}</p>
        </div>
      </div>
    </article>
  )
